...
use crate::application::ports::{
    AuthorizationCodeRepository, MerchantRepository, NewAuthorizationCode, NewOAuthClient,
    NewRefreshToken, OAuthClientRepository, RefreshTokenRepository, RepoError,
};
use crate::application::use_cases::MerchantAuthUseCases;
...
/// OAuth 2.1 + PKCE authorization server scoped to marchand accounts: an MCP
/// client (Claude Code, Claude Desktop, claude.ai) connects as a marchand to
/// manage their boutique, the same way `/marchands/connexion` would, but
/// with a real "Connect" flow instead of a manually pasted token. Repris du
/// pattern "mcp-oauth-maison" d'Elevia (https://github.com/gilmry/elevia).
pub struct OAuthUseCases {
    ...
    merchant_auth_use_cases: Arc<MerchantAuthUseCases>,
}
...
impl OAuthUseCases {
    ...
    async fn exchange_code(&self, dto: TokenRequestDto) -> Result<TokenResponseDto, OAuthError> {
        ...
        let merchant = self
            .merchant_repo
            .find_by_id(record.marchand_id)
            .await?
            .ok_or(OAuthError::InvalidGrant)?;

        self.issue_token_pair(&merchant, &record.client_id).await
    }
    ...
    async fn issue_token_pair(
        ...
        let access_token = self
            .merchant_auth_use_cases
            .mint_token(merchant, Duration::minutes(ACCESS_TOKEN_TTL_MINUTES))
