    ...
    async fn create(&self, new: NewAuthorizationCode) -> Result<(), RepoError> {
        sqlx::query(
            "INSERT INTO oauth_authorization_codes
                 (code, client_id, marchand_id, redirect_uri, code_challenge, expires_at)
             VALUES ($1, $2, $3, $4, $5, $6)",
        )
        .bind(new.code)
        .bind(new.client_id)
    ...
    async fn consume(&self, code: &str) -> Result<Option<AuthorizationCode>, RepoError> {
        sqlx::query_as::<_, AuthorizationCode>(
            "UPDATE oauth_authorization_codes SET used = true
             ...
             RETURNING code, client_id, marchand_id, redirect_uri, code_challenge, expires_at, used",
        )
        .bind(code)
        .fetch_optional(&self.pool)
        .await
...
impl RefreshTokenRepository for PostgresRefreshTokenRepository {
    async fn create(&self, new: NewRefreshToken) -> Result<(), RepoError> {
        sqlx::query(
            "INSERT INTO oauth_refresh_tokens (token_hash, client_id, marchand_id, expires_at)
             VALUES ($1, $2, $3, $4)",
        )
        .bind(new.token_hash)
        .bind(new.client_id)
    ...
    async fn find_valid(&self, token_hash: &str) -> Result<Option<RefreshToken>, RepoError> {
        sqlx::query_as::<_, RefreshToken>(
            "SELECT token_hash, client_id, marchand_id, expires_at, revoked
