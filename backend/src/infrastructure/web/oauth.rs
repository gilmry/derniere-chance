...
//! offer a real "Connect" button instead of asking the marchand to paste a
//! JWT that expires in 24h. Fully self-hosted: no external identity
//! provider, delegates the actual credential check to the same
//! email/password `marchands` table `/marchands/connexion` already uses.
//!
...
pub async fn metadata(req: HttpRequest) -> HttpResponse {
    ...
    HttpResponse::Ok().json(json!({
        ...
        "scopes_supported": ["derniere-chance"],
...
<head>
<meta charset="utf-8">
<title>Connexion DernièreChance</title>
<style>
...
<body>
<h1>DernièreChance</h1>
<p><strong>{client_name}</strong> demande à accéder à votre compte marchand DernièreChance.</p>
{error_html}
